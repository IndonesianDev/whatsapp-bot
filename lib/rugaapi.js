...
const joox = (judul) => new Promise((resolve, reject) => {
    ...
    fetchJson('https://api.vhtear.com/music?query=' + judul + '&apikey=' + vhtearkey)
        .then((result) => resolve(result))
        .catch((err) => {reject(err)
})
})

const playt = (judul) => new Promise((resolve, reject) => {
    console.log(`Getting play music for ${judul}...`)
    fetchJson('https://api.vhtear.com/ytmp3?query=' + judul + '&apikey=' + vhtearkey)
        .then((result) => resolve(result))
        .catch((err) => {reject(err)
})
})
...
const insta = (url) => new Promise((resolve, reject) => {
    ...
    fetchJson(`https://api.vhtear.com/instadl?link=${url}&apikey=${vhtearkey}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
...
const pasangan = (nama, pasangan) => new Promise((resolve, reject) => {
    ...
    fetchJson('https://api.vhtear.com/primbonjodoh?nama=' + nama + '&pasangan=' + pasangan + '&apikey=' + vhtearkey)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
const ytdl = (url) => new Promise((resolve, reject) => {
    console.log(`Getting YouTube media from ${url}`)
    fetchJson('https://api.i-tech.id/dl/yt?key=' + apitech + '&link=' + url)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
...
const ytPlay = (query) => new Promise((resolve, reject) => {
    ...
    fetchJson(`https://api.vhtear.com/ytmp3?query=${query}&apikey=${vhtearkey}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
...
const jam = async (teks) => new Promise((resolve, reject) => {
    axios.get(`https://api.i-tech.id/tools/jam?key=${apitech}&kota=${teks}`)
    .then((res) => {
        const text = `Timezone: ${res.data.timezone}\n\nTanggal: ${res.data.date}\nJam: ${res.data.time}`
        resolve(text)
    })
    .catch((err) =>{
        reject(err)
...
const sms = async (to,url) => new Promise((resolve, reject) => {
	axios.get(`https://api.i-tech.id/special/sms?key=${apitech}&no=${to}&msg=${url}`)
	.then((res) => {
		resolve(res.data)
	})
	.catch((err) => {
...
const hilih = async (kalimat) => new Promise((resolve, reject) => {
	axios.get(`https://api.i-tech.id/tools/hilih?key=${apitech}&kata=${kalimat}`)
	.then((res) => {
		resolve(res.data.result)
	})
	.catch((err) => {
